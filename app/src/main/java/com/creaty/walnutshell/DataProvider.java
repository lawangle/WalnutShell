...
public class DataProvider extends ContentProvider {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE " + NewsTableMetaData.TABLE_NAME + " ("
					...
					+ NewsTableMetaData.FREQENCE + " INTEGER NOT NULL,"
			...
			db.execSQL("CREATE TABLE " + SourceTableMetaData.TABLE_NAME + " ("
					...
					+ SourceTableMetaData.SOURCE_STRATEGY + " INTEGER NOT NULL,"
			...
			db.execSQL("CREATE TABLE " + EntryTableMetaData.TABLE_NAME + " ("
					...
					+ EntryTableMetaData.CONTENT_ADDRESS
					+ " TEXT NOT NULL," + EntryTableMetaData.DESCRIPTION
					+ " TEXT," + EntryTableMetaData.AUTHOR + " TEXT,"
	...
	public int delete(Uri uri, String where, String[] whereArgs) {
		...
		switch (sUriMatcher.match(uri)) {
		case INCOMING_SOURCE_COLLECTION_URI_INDICATOR:
			count = db.delete(SourceTableMetaData.TABLE_NAME, where, whereArgs);
			break;
		...
		case INCOMING_ENTRY_COLLECTION_URI_INDICATOR:
			count = db.delete(EntryTableMetaData.TABLE_NAME, where, whereArgs);
			break;
		...
		case INCOMING_SINGLE_NEWS_URI_INDICATOR:
			rowId = uri.getPathSegments().get(1);
			count = db.delete(
					NewsTableMetaData.TABLE_NAME,
					NewsTableMetaData._ID
							+ "="
							+ rowId
							+ (!TextUtils.isEmpty(where) ? " AND (" + where
									+ ')' : ""), whereArgs);
	...
	public int update(Uri uri, ContentValues values, String where,
		...
		switch (sUriMatcher.match(uri)) {
		case INCOMING_SOURCE_COLLECTION_URI_INDICATOR:
			count = db.update(SourceTableMetaData.TABLE_NAME, values, where,
					whereArgs);
			break;
		...
		case INCOMING_ENTRY_COLLECTION_URI_INDICATOR:
			count = db.update(EntryTableMetaData.TABLE_NAME, values, where,
					whereArgs);
			break;
		...
		case INCOMING_NEWS_COLLECTION_URI_INDICATOR:
			count = db.update(NewsTableMetaData.TABLE_NAME, values, where,
					whereArgs);
			break;
